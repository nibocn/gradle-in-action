package me.nibo.gradle.plugins.cloudbees

import me.nibo.gradle.plugins.cloudbees.tasks.CloudBeesTask
import me.nibo.gradle.plugins.cloudbees.tasks.app.CloudBeesAppDeployWar
import me.nibo.gradle.plugins.cloudbees.tasks.app.CloudBeesAppInfo
...
class CloudBeesPlugin implements Plugin<Project> {
    ...
    private void addAppTasks(Project project) {
        ...
        project.tasks.withType(CloudBeesAppDeployWar).whenTaskAdded { task ->
            task.conventionMapping.appId = { getAppId(project) }
            task.conventionMapping.message = {
                project.hasProperty('message') ? project.message : null
            }
    ...
    private String getAppId(Project project) {
        project.hasProperty('appId') ?
                project.appId : project.extensions.findByName(EXTENSION_NAME).appId
    }

    private File getWarFile(Project project) {
        project.hasProperty('warFile') ?
