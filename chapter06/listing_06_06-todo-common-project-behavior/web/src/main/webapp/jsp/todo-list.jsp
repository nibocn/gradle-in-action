        ...
        <ul id="todo-list">
            <c:forEach var="toDoItem" items="${toDoItems}" varStatus="status">
                <li id="toDoItem_${status.count}"
                    class="<c:if test="${toDoItem.completed}">completed</c:if>"
                    ...
                    <div class="view">
                        <form id="toggleForm_${status.count}" action="<c:url value="toggleStatus"/>"
                              method="POST">
                    ...
                    <form id="updateForm_${status.count}" action="<c:url value="update"/>"
                          method="POST">
                        <input type="hidden" name="id" value="${toDoItem.id}"/>
                        <input type="hidden" name="filter" value="${filter}"/>
    ...
    <footer id="footer">
        <c:if test="${stats.all > 0}">
            ...
            <ul id="filters">
                <li>
                    <a
                            <c:if test="${filter == 'all'}">class="selected"</c:if>
                            href="<c:url value="/all"/>">All</a>
                </li>
                <li>
                    <a
                            <c:if test="${filter == 'active'}">class="selected"</c:if>
                            href="<c:url value="active"/>">Active</a>
                </li>
                <li>
                    <a
            ...
            <c:if test="${stats.completed > 0}">
                <form action="<c:url value="clearCompleted"/>" method="POST">
                    <input type="hidden" name="filter" value="${filter}"/>
                    <button id="clear-completed">Clear completed (<c:out
